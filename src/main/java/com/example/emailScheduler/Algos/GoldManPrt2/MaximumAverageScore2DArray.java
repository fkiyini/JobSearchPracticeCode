...
class MaximumAverageScore2DArray {
    ...
    public static double getMaxAverageScore(String[][] scores) {
        if (scores == null)
            return -1;
        ...
        return map.values()
                .stream()
                .map(l -> l.stream().mapToInt(i -> i).average())
                .mapToDouble(i -> i.getAsDouble())
                .max().getAsDouble();
//        List<Integer> newList = new ArrayList<>();
//        map.values().stream().forEach(maxAverage>getAverageScoreOfStudent(newList)?maxAverage:getAverageScoreOfStudent(newList));
//        return maxAverage;
    }
